...
final routeProvider = Provider<GoRouter>((ref) {
  final router = GoRouter(
    redirect: (context, state) {
      ...
      if (initialState is SplashGoToShowCaseState &&
          !state.matchedLocation.startsWith('/${AppRoutes.showcase}')) {
        return '/${AppRoutes.showcase}';
      }

      if (initialState is SplashGoToLoginState &&
          !state.matchedLocation.startsWith('/${AppRoutes.login}')) {
        return '/${AppRoutes.login}';
      }
      if (initialState is SplashGoToOnboardingState &&
          !state.matchedLocation.startsWith('/${AppRoutes.onboarding}')) {
        return '/${AppRoutes.onboarding}';
      }

      if (initialState is SplashGoToHomeState &&
          !state.matchedLocation.startsWith('/${AppRoutes.home}')) {
        return '/${AppRoutes.home}';
      }

      if (state.matchedLocation.isEmpty) return '/${AppRoutes.home}';
      return null;
    },
